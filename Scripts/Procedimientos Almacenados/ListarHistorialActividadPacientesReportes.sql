...
DROP PROCEDURE ListarHistorialActividadPacientesReportes
GO



CREATE PROCEDURE ListarHistorialActividadPacientesReportes
		@NumeroPaciente	INT,
...
BEGIN
	...
	SET @AUX = ' '
	SET @Consulta= 'SELECT PD.*, SS.*, AT.NombreActividad 
	FROM PacientesDatos  PD 
	INNER JOIN dbo.Actividades SS  
	...
	IF(@NumeroPaciente IS NOT NULL)
		SET @Filtro = @Filtro + ' SS.NumeroPaciente = ' + CAST(@NumeroPaciente AS VARCHAR(10)) + ' AND'
	IF(@CodigoActividadTipo IS NOT NULL)
		SET @Filtro = @Filtro + ' SS.CodigoActividadTipo = ' + CAST(@CodigoActividadTipo AS VARCHAR(10)) + ' AND'
	...
	SET @Consulta = @Consulta + ' ORDER BY ' + CASE @TipoOrden WHEN 'F' THEN 'SS.FechaActividad ' WHEN 'H' THEN 'PD.HClinico ' WHEN 'A' THEN 'SS.CodigoActividadTipo ' ELSE 'NombreCompletoPaciente ' END
